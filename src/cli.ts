...
async function makeContext(engine: BrainEngine, params: Record<string, unknown>): Promise<OperationContext> {
  ...
  try {
    ...
    // Company context is representational only; CLI behavior must not depend
    // on company policy config being present or valid before permission enforcement.
  }
  return ctx;
}
...
const THIN_CLIENT_REFUSED_COMMANDS = new Set([
  ...
  'dream', 'transcripts', 'storage', 'company',
...
const THIN_CLIENT_REFUSE_HINTS: Record<string, string> = {
  ...
  company: 'company ingestion/extraction/retrieval/follow-up drafting, policy inspection, and enforcement handoff are local/manual for the trusted-workspace pilot. Run it on the host; hosted writes and external execution stay disabled, and policy is not yet fully enforced.',
...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (command === 'company' && (args.includes('--help') || args.includes('-h'))) {
    const { runCompany } = await import('./commands/company.ts');
    await runCompany(null, args);
    return;
  }
  if (
    command === 'company'
    && ['enforcement-handoff', 'handoff'].includes(args[0] ?? '')
  ) {
    const { runCompany } = await import('./commands/company.ts');
    await runCompany(null, args);
    return;
...
IMPORT/EXPORT
  ...
  company ingest meeting <file>      Local/manual company transcript ingest
        [--doc PATH] [--no-embed]    Trusted workspace only; no live connectors
  company ingest doc <file>          Local/manual linked-doc ingest
  company extract all|<slug>         Local decisions/commitments/actions
  company query <question>           Local company retrieval with citations
  ...
  company policy seed|grants|context Local/admin policy inspection; not fully enforced
  company enforcement-handoff        Permission-enforcement hook plan; not enforcement
