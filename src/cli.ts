...
function printCliOnlyHelp(command: string) {
  if (command === 'init') {
    ...
    console.log('');
    console.log('Options:');
    console.log('  --company        Trusted workspace company brain; reviewed hosted MCP permissions, no DB ACL/RLS claim');
...
const THIN_CLIENT_REFUSE_HINTS: Record<string, string> = {
  ...
  company: 'company ingestion/extraction/retrieval/follow-up drafting, policy inspection, and permission status. Run local/admin workflows on the host; reviewed hosted MCP operations enforce application-layer permissions, while broad hosted writes and external execution stay disabled.',
...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (
    command === 'company'
    && ['enforcement-handoff', 'handoff', 'permission-status', 'permission'].includes(args[0] ?? '')
  ) {
    const { runCompany } = await import('./commands/company.ts');
    await runCompany(null, args);
    return;
